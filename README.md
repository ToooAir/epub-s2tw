...
| --- | --- | --- |
| **翻譯引擎** | **神經機器翻譯 (Google Translate)**<br>基於整段數百字的上下文進行語意對齊轉換。能精確辨識輕小說中的俚語、自創詞與語法。 | **靜態 Trie Tree 文字對應**<br>基於單字或固定詞組的替換，遇到字典未收錄的新穎網路詞彙或特殊修辭時較難靈活應對。 |
| **後處理防護** | **逆向防禦網 + 台灣過濾鏡**<br>將 OpenCC 字典轉化為「捕捉 Google 潛在錯誤的陷阱」。利用 `STPhrases` 偵測轉換錯詞並導正，外加 `TWVariants` 強制洗滌成台灣本地標準字 (如裏→裡)。 | **忠實的一對一轉換**<br>單純的繁簡字體映射。若原始小說來源檔本身帶有錯別字，轉換後仍會忠實保留原樣。 |
| **全域一致性校正** | **基於 N-gram 的智慧收斂與多數決**<br>具備整本電子書的全域記憶。能自動將不同章節中被機翻不穩定轉化的名詞變體（例如：雪之下 vs 雪底下），透過演算法對齊並收斂成最通用的單一版本。 | **無狀態 (Stateless) 的字元對映**<br>第一行與第一千行的轉換完全獨立，不受語意影響，無需且無法干預前後文連貫性或名詞的翻譯統一性。 |
| **幻覺抵禦與防撞** | **八段防護網與絕對真理法則**<br>當 AI 發生 51% 比例的大規模幻覺（如把「政近一行」超譯為「政近一步」），或遭遇原著雙胞胎名字陷阱時，系統具備「段落競爭對齊」與「全域合規防撞網」，精準抹除機翻幻覺的同時守護原著彩蛋。 | **專注於字體結構替換**<br>按部就班地執行字體轉換，不涉及語意判讀，自然也不負責處理機翻獨有的「幻覺產生」與「同一語意多譯名衝突」等問題。 |
| **極限斷句容錯** | **自動碎紙重譯自救機制**<br>針對 NMT 因遇到輕小說中常見的高重複疊詞（如連續的「啊啊啊」）而導致的 API 截斷不吐字現象，系統能觸發標點符號自動降級拆分，挽救丟失的文句。 | **穩定的純本地運算**<br>完全本地端執行，無須顧慮 API 截斷、網路逾時或文本過長的問題，執行效率極高。 |
