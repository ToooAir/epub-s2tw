...
7. **絕對真理法則（抵禦機翻引擎的 51% 攻擊）**：一般的多數決演算法有致命缺陷——若 Google 抽風，高達四分之三的機率把主角團「政近一行」超譯成「政近一步」，民主演算法就會瞎了眼把正解當成錯字殺掉。系統為此導入了超越多數決的「數學真理判斷」：只要原文全為沒有繁簡歧義的固定字（如 `政近一行` 或 `艾莉莎`），那麼它自己就是宇宙唯一的絕對真理！系統會直接褫奪 AI 的民主投票權，無視 33% 容錯上限，無情且專制地把高達 80% 的腦霧幻覺全部強制改回原著的真理字眼中！
